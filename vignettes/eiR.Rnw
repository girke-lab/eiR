...
The \texttt{runId} parameter is required to determine which embedded database to use. 
As with \texttt{eiMakeDb}, the \texttt{distance}
parameter may be given if desired, it will default to the
Tanimoto Coefficient otherwise.
...
Similarities can be computed by setting \texttt{asSimilarity} to TRUE.

Then you can perform a query as follows: 

<<term=false,fig=true>>=
#find compounds similar to each query
result=eiQuery(runId,sdfsample[45],K=10)
...
\texttt{queries} parameter, there is an \texttt{additions} parameter,
...
data to evaluate the performance of the current embedding. It takes
the usual \texttt{runId} parameter, as well as on optional distance function.
It also
